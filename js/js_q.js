...
function displayPrevNext(temp) {
    $.getJSON('./json/question.json', function(data) {
        ...
        if (quiz == undefined)
            quiz= data;
        if (temp < 0) {
            alert("Warning: Sorry no Previous questions!!!");
            return;
        } else if (temp >= 5) {
            alert("Warn:You have answered the last question, click Finish quiz!!!");
            return;
        }
...
function finishQuiz() {
    ...
    if (finishedQuiz === 0) {
        ...
        alert("You answered " + correctAnswers + "  questions correctly which is " + totalPercentage + "%");

        var result = totalPercentage >= 60 ? "passed it!!!" : "failed";
        var greeting = result === "passed" ? "Congratulations" : "sorry";
        alert(greeting + "  You " + result + " the tnbQuiz");
        finishedQuiz = 1;
        localStorage.setItem('quizStarted',"False");
    }
    if (finishedQuiz > 0) {
        window.open("https://thenewboston.com/account.php", "_self");
    }
